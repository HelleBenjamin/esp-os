...
void kernel_functions::init_kernel() {
    ...
    Serial.begin(115200);
    kernel.kernel_log.push_back("EEPROM: " + String(EEPROM_SIZE) + " bytes");
    kernel.kernel_log.push_back(kernelversion);
    kernel.kernel_log.push_back(drivers);
    delay(1);
...
void kernel_functions::kernel_log_manager() {
}
...
void task_scheduler::create_task(void (*task_function)(), unsigned long interval, const char* process_name, int priority) {
    ...
    kernel.kernel_log.push_back("Task: " + String(process_name) + " created with interval: " + String(interval) + " and priority: " + String(priority));
}
...
void task_scheduler::taskkill(int pid) {
    for (int i = 0; i < num_tasks; i++) {
        if (tasks[i].pid == pid) {
            ...
            num_tasks--;
            kernel.kernel_log.push_back("Ended Task: " + String(pid));
            return;
        }
        else {
            Serial.println("Task: " + String(pid) + " not found");
            kernel.kernel_log.push_back("Failed to kill Task: " + String(pid));
            return;
        }
    }
...
void task_scheduler::change_priority(int pid, int new_priority) {
    tasks[pid].priority = new_priority;
    kernel.kernel_log.push_back("Task: " + String(pid) + " priority changed to: " + String(new_priority));
}
...
void syscalls::print_log() {
    for (int i = 0; i < kernel.kernel_log.size(); i++) {
        Serial.println(kernel.kernel_log[i]);
    }
}
